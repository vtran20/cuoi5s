...
<c:set value="${fn:replace(lastid,'.html','')}" var="lastid"/>
<c:set value="${fn:split(lastid,'/')}" var="lastids"/>

<c:if test="${fn:length(lastids) == 1}">
    ...
    <spring:eval expression="serviceLocator.getCategoryDao().findById(T(java.lang.Long).valueOf(categoryId))" var="subCategory" />
</c:if>
<c:if test="${fn:length(lastids) == 2}">
    <c:set value="${lastids[1]}" var="subcategoryuri"/>
    <c:set value="${fn:split(subcategoryuri,'-')}" var="list"/>
    <c:set value="${list[fn:length(list)-1]}" var="subCategoryId"/>
    <spring:eval expression="serviceLocator.getCategoryDao().findById(T(java.lang.Long).valueOf(subCategoryId))" var="subCategory" />
    <c:set value="${lastids[0]}" var="categoryuri"/>
    <c:set value="${fn:split(categoryuri,'-')}" var="list"/>
    <c:set value="${list[fn:length(list)-1]}" var="categoryId"/>
    <spring:eval expression="serviceLocator.getCategoryDao().findById(T(java.lang.Long).valueOf(categoryId))" var="category" />
</c:if>
<c:if test="${fn:length(lastids) == 3}">
    <c:set value="${lastids[2]}" var="subcategoryuri"/>
    <c:set value="${fn:split(subcategoryuri,'-')}" var="list"/>
    <c:set value="${list[fn:length(list)-1]}" var="subCategoryId"/>
    <spring:eval expression="serviceLocator.getCategoryDao().findById(T(java.lang.Long).valueOf(subCategoryId))" var="subCategory" />
    <c:set value="${lastids[1]}" var="categoryuri"/>
    <c:set value="${fn:split(categoryuri,'-')}" var="list"/>
    <c:set value="${list[fn:length(list)-1]}" var="categoryId"/>
    <spring:eval expression="serviceLocator.getCategoryDao().findById(T(java.lang.Long).valueOf(categoryId))" var="category" />
    <c:set value="${lastids[0]}" var="parcategoryuri"/>
    <c:set value="${fn:split(parcategoryuri,'-')}" var="list"/>
    <c:set value="${list[fn:length(list)-1]}" var="parCategoryId"/>
    <spring:eval expression="serviceLocator.getCategoryDao().findById(T(java.lang.Long).valueOf(parCategoryId))" var="parCategory" />
</c:if>
...
<spring:eval expression="serviceLocator.getSearchService().getProductsBySubCategoryUsingLucene(subCategory.id.toString(), site, refinementMap, start, itemPerPage, sortField, reverse)" var="mapResult" />
<c:choose>
                ...
                <div class="col-md-9">
                    <div class="row margin-bottom-5">
                        <div class="col-sm-7 result-category">
                            ...
                            <small class="shop-bg-red badge-results">${mapResult.count} <fmt:message key="category.product"/></small>
                        </div>
                        <div class="col-sm-5">
