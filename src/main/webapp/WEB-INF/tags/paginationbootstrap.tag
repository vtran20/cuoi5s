...
<c:if test="${totalItems > itemsPerPage}">
    <div class="pagination pagination-right">
        <ul>
            ...
            <c:if test="${currPage > 1}">
                <li class="active"><a href="${browseruri}?${query}"><fmt:message key="pagination.previous"/></a></li>
            </c:if>
            ...
            <c:forEach begin="${begin}" end="${end}" varStatus="page">
                <c:if test="${currPage == page.index}">
                    <li class="disabled"><a href="#">${page.index}</a></li>
                </c:if>
                <c:if test="${!(currPage == page.index)}">
                    <spring:eval expression="params.add('page',page.index).toString()" var="query"/>
                    <li class="active"><a href="${browseruri}?${query}">${page.index}</a></li>
                </c:if>
            </c:forEach>
            ...
            <c:if test="${currPage * itemsPerPage < totalItems}">
                ...
                <li class="active"><a href="${browseruri}?${query}"><fmt:message key="pagination.next"/></a></li>
            </c:if>
            <c:if test="${currPage * itemsPerPage >= totalItems}">
