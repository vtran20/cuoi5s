...
<div class="col-sm-4">
    <div class="product-img product-img-brd">
        ...
        <c:if test="${!empty image}">
            <c:choose>
                <c:when test="${!empty image.crop}"><c:set var="imageUrl" value="${imageServer}/get/${image.uri}.jpg?op=crop|${image.crop}&op=scale|x328"/></c:when>
                <c:otherwise><c:set var="imageUrl" value="${imageServer}/get/${image.uri}.jpg?op=scale|x328&op=crop|0,0,262,328"/></c:otherwise>
            </c:choose>
        </c:if>
