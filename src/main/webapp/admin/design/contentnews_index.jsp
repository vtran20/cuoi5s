...
<div class="page-header">
    <a class="btn btn-xs btn-info" href="#modal-form" role="button" data-toggle="modal" data-title="<fmt:message key="common.add.new"/> ${row.title}" data-page="/admin/design/contentnews_popup.html?rowId=${rowId}" data-target="#modal-form">
...
<div class="row">
    <div class="col-xs-12">
        <table id="simple-table-content" class="table table-striped table-bordered table-hover sorted_table">
            <thead>
            <tr>
                <th><fmt:message key="admin.design.web.news.list"/></th>
                ...
                <tr id="${partContent.current.id}">
                    <td>
                        <c:if test="${! empty partContent.current.header}">
                            <spring:eval expression="serviceLocator.newsDao.findById(T(java.lang.Long).valueOf(partContent.current.header))" var="news"/>
                            <h5><c:out value="${news.title}"/></h5>
                        </c:if>
                    </td>
                    <%--<c:choose>--%>
