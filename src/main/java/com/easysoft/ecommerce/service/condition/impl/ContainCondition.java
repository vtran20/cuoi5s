...
/**
 * Expession: 'PARAM_1'.contains('DYNAMIC_DATA')
 * 
 * User: Vu Tran
 * Date: Dec 19, 2010
 * Time: 3:13:11 PM
 ...
 */
public class ContainCondition extends ConditionSupport implements Serializable {

    private static final Logger LOGGER = LoggerFactory.getLogger(ContainCondition.class);

    @Override
    public String execute(SessionObject sessionObject, ItemMap itemMap, User user, String expression) throws Exception {
        String city = sessionObject.getAddresses().getShippingAddress().getCity();
        if (!StringUtils.isEmpty(expression) && !StringUtils.isEmpty(city)) {
            expression = expression.replaceAll("SHIPPING_CITY", city);
        }
        return expression;
    }
}
